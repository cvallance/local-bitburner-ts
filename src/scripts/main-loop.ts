...
const remoteExec = (
    ns: NS,
    freeServers: FreeServer[],
    script: string,
    threads: number,
    serverToRunAgainst: string,
    runCount: number
) => {
    if (threads <= 0) return
...
const weakenThreadMaths = (
    ns: NS,
    homeServer: Server,
    amountToWeaken: number,
    maxThreads: number
): ThreadCounts => {
...
const growThreadMaths = (
    ...
    const wantedGrow = Math.ceil(
        ns.growthAnalyze(serverToHack.hostname, desiredGrowMulti + 0.1, 1)
    )
...
const hackThreadMaths = (
    ...
    const wantedWeakenThreads = weakenThreadMaths(
        ns,
        homeServer,
        secUp,
        maxThreads
    ...
    if (
        wantedHackThreads + wantedGrowThreads + wantedWeakenThreads <=
        maxThreads
    ) {
        return {
...
const hackGrowWeaken = async (ns: NS, servers: Server[]) => {
    ...
    const serversToWorkOn = allServers.filter(
        (x) => !processes.some((y) => y.args.some((z) => z == x.hostname))
    )
    // Can weaken and grow
    const serversToWeakenAndGrow = serversToWorkOn.filter(
        (x) => x.hasAdminRights && x.moneyMax
    )
    // Can also hack
    const serversToHack = serversToWeakenAndGrow
        .filter((x) => hackLvl >= x.requiredHackingSkill)
        .sort((x, y) => y.moneyMax - x.moneyMax)

    let serverToHackCount = Math.min(
        Math.ceil(freeThreadCount() / 500),
        serversToHack.length
    )
    for (const serverToHack of serversToHack) {
        ...
        if (currDifficulty != minDifficulty) {
            const threadMaths = weakenThreadMaths(
                ns,
                homeServer,
                currDifficulty - minDifficulty,
                maxThreads
            )
            remoteExec(
                ns,
        ...
        if (currMoney != maxMoney) {
            const threadMaths = growThreadMaths(
                ns,
                homeServer,
                serverToHack,
                maxMoney,
        ...
        // Should hack
        const threadMaths = hackThreadMaths(
            ns,
            homeServer,
            serverToHack,
            maxThreads
...
const buyServer = async (ns: NS, servers: Server[]) => {
    ...
    const purchasedServers = servers.filter(
        (x) => x.purchasedByPlayer && x.hostname != 'home'
    )

    // Buy new servers while we have enough money AND we haven't reached the limit
    if (
        ns.getServerMoneyAvailable('home') >
        ns.getPurchasedServerCost(purchaseServerRam)
    ) {
        if (purchasedServers.length == purchaseLimit) {
            const wrongRamServers = purchasedServers.filter(
                (x) => x.maxRam != purchaseServerRam
            )
            ...
            ns.tprint(
                `Killing server ${serverToKill} to make room for another one`
            )
