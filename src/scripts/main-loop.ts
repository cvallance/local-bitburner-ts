...
const growThreadMaths = (ns: NS, homeServer: Server, serverToHack: Server, maxMoney: number, currMoney: number, maxThreads: number): ThreadCounts => {
...
const hackGrowWeaken = async (ns: NS, servers: Server[]) => {
    ...
    const freeServers: FreeServer[] = serversCanRun.map((s) => {
		...
		return {hostname: s.hostname, freeThreads: Math.floor(freeRam/ramCost)}
	}, [])
